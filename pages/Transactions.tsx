...
// ✅ API CONFIGURATION
// ✅ API CONFIGURATION removed in favor of centralized api service 
...
export const Transactions: React.FC<TransactionsProps> = ({ user }) => {
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      try {
        const bankResponse = await api.get('/banks', {
          params: { userId: user.id }
        });
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {filteredTransactions.length > 0 ? filteredTransactions.map((tx) => (
                <tr key={tx._id || tx.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${selectedIds.has(tx._id || tx.id) ? 'bg-blue-50 dark:bg-blue-900/10' : ''}`}>
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs font-bold px-2 py-1 rounded ${tx.paymentMethod === PaymentMethod.GPAY ? 'bg-blue-100 text-blue-700' :
