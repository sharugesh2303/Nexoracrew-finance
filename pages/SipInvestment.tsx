...
export const SipInvestment: React.FC<SipProps> = ({ user }) => {
    ...
    const fetchMarketData = async () => {
        try {
            const res = await api.get('/market/live');
    ...
    const handleCreatePlan = async () => {
        ...
        try {
            await api.post('/sip-plans', { ...newPlan, active: true });
    ...
    const handleDeletePlan = async (id: string) => {
        if (confirm("Stop tracking this SIP Plan?")) {
            try { await api.delete(`/sip-plans/${id}`); fetchData(); setSelectedPlan(null); } catch (error) { }
        }
    };
    ...
    const handleDeleteHistory = async (id: string) => {
        if (confirm("Delete this payment record? This will adjust your total wealth.")) {
            try {
                ...
                const txRes = await api.get('/transactions', { params: { userId: user.id } });
    ...
    const handleMarketSearch = async (e: React.FormEvent) => {
        ...
        setIsSearching(true);
        try {
            const res = await api.get('/market/search', { params: { q: marketSearchQuery } });
            setMarketIndices(res.data);
